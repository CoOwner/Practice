...
public class KitManager extends Manager
{
    ...
    public void loadKits() {
        ...
        for (final String kitName : arenaSection.getKeys(false)) {
            ...
            final ItemStack[] mainContents = (ItemStack[]) ((List)arenaSection.get(kitName + ".mainContents")).toArray(new ItemStack[0]);
            final ItemStack[] armorContents = (ItemStack[]) ((List)arenaSection.get(kitName + ".armorContents")).toArray(new ItemStack[0]);
    ...
    public void openEditiKitsInventory(final Player player, final Kit defaultKit, final PlayerKit kit) {
        final Inventory inventory = Bukkit.createInventory((InventoryHolder)null, 54, ChatColor.GOLD + "Editing Kit");
