...
import com.ubuntu.practice.tournament.tasks.*;
...
public class Tournament
{
    ...
    public Tournament(final int maximumPerTeam, final Kit defaultKit, final boolean playerTournament) {
        this.plugin = uPractice.getInstance();
        ...
        this.taskId = this.getPlugin().getServer().getScheduler().runTaskTimer((Plugin)this.plugin, (BukkitRunnable)new TournamentTask(this), 200L, 200L).getTaskId();
    ...
    public void generateRoundMatches() {
        ...
        else if (this.teams.size() <= 8 && this.teams.size() > 4 && this.tournamentStage != TournamentStage.QUARTER_S) {
            this.setTournamentStage(TournamentStage.QUARTER_S);
            if (!this.started) {
                this.started = true;
            }
            this.plugin.getServer().broadcastMessage(Lang.getLang().getLocalized((CommandSender)Bukkit.getConsoleSender(), "TOURNAMENT_ROUNDS").replace("%max_per_team%", String.valueOf(this.maximumPerTeam)).replace("%round%", "Quarter s"));
        }
        else if (this.teams.size() <= 4 && this.teams.size() > 2 && this.tournamentStage != TournamentStage.SEMI_S) {
            this.setTournamentStage(TournamentStage.SEMI_S);
            if (!this.started) {
                this.started = true;
            }
            this.plugin.getServer().broadcastMessage(Lang.getLang().getLocalized((CommandSender)Bukkit.getConsoleSender(), "TOURNAMENT_ROUNDS").replace("%max_per_team%", String.valueOf(this.maximumPerTeam)).replace("%round%", "Semi-s"));
        }
        else if (this.teams.size() == 2 && this.tournamentStage != TournamentStage.S) {
            this.setTournamentStage(TournamentStage.S);
            if (!this.started) {
                this.started = true;
            }
            this.plugin.getServer().broadcastMessage(Lang.getLang().getLocalized((CommandSender)Bukkit.getConsoleSender(), "TOURNAMENT_ROUNDS").replace("%max_per_team%", String.valueOf(this.maximumPerTeam)).replace("%round%", "Grand-"));
        }
